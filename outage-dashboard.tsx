...
const InteractiveReport = dynamic(
  () =>
    import("./components/interactive-report").then(
      (mod) => (mod.InteractiveReport ?? mod.default) as React.ComponentType<any>,
    ),
  { ssr: false, loading: () => <div className="h-96 rounded-lg bg-muted animate-pulse" /> },
)
                              ...
                              {loading ? (
                                [...Array(4)].map((_, i) => (
                                  <div key={i} className="flex">
                                    <div className="w-80 xl:w-96 2xl:w-[400px] h-14 rounded bg-muted animate-pulse mr-2"></div>
